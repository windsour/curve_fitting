...
def alpha_model(t, E, n, T, A):
    ...
    for i in range(1, len(t)):
        d_alpha = d_alpha_dt(E, n, alpha[-1], T) * dt
...
params, covariance = curve_fit(
    lambda t, E, n: alpha_model(t, E, n, T, A),
    t_data_filtered,
    alpha_data_filtered,
    p0=initial_guess
)
...
# 输出拟合结果
E_fit, n_fit = params
print(f"Fitted parameters: E = {E_fit:.2f}, n = {n_fit:.2f}")

# 使用拟合参数计算拟合曲线（完整时间范围，用于绘图）
alpha_fit_full = alpha_model(t_data, E_fit, n_fit, T, A)
...
plt.plot(t_data, alpha_fit_full, '-', label=f'Fitted curve (E={E_fit:.2f}, n={n_fit:.2f})')
...
plt.legend()
plt.grid(True)
plt.text(0.5, 0.95, f'R² = {r2:.4f}', transform=plt.gca().transAxes,
