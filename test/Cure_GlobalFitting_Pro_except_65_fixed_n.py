...
# 调试：检查预测长度是否匹配
# ========================
initial_guess = [70000, 1e5]  # E 和 A（n 固定为 0.9）
...
# 初始猜测和边界（只对 E 和 A）
# ========================
bounds = ([0, 0], [200000, 1e15])  # E ∈ [0, 200000], A ∈ [0, 1e15]
...
if result.success:
    ...
    # 计算 R²（全局）
    # ========================
    y_pred = model_prediction([E_fit, A_fit], t_list, T_list, alpha0_list)
    ...
    plt.title('Kinetic Fitting with n = 0.9 (Global Fit)')
    plt.legend()
