...
T_65 = 348.15  # 65 + 273.15

# ========================
# 🔍 数据筛选：只保留稳定阶段之后的数据
# ========================
# threshold_55 = 200000  # s (可调整)
# threshold_60 = 150000  # s (可调整)
# threshold_65 = 120000  # s (可调整)

threshold_55 = 20000  # s (可调整)
threshold_60 = 20000  # s (可调整)
threshold_65 = 20000  # s (可调整)

mask_55 = t_55 >= threshold_55
mask_60 = t_60 >= threshold_60
mask_65 = t_65 >= threshold_65  # 使用 >= 避免浮点误差
...
# 调试：检查预测长度是否匹配
# ========================
initial_guess = [50000, 0.9, 1e5]  # 建议 A 更大，如 1e8
...
if result.success:
    ...
    # ========================
    print("\n🧪 分组 R²:")
    labels = ['55°C (328.15 K)', '60°C (333.15 K)', '65°C (348.15 K)']
    for i in range(3):  # 改为 3 组
    ...
    for i in range(3):  # 绘制三组数据
        t_full = [t_55, t_60, t_65][i]
    ...
    plt.title('Kinetic Fitting on Stable Phase (Global Fit) - 55°C, 60°C, 65°C')
    plt.legend()
